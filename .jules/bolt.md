...
## 2024-05-24 - Zero-Allocation String Parsing Optimization
**Learning:** When optimizing `String.split()` by replacing it with `indexOf()` and `substring()`, we avoid array allocations and regex recompilation overhead. However, we must be careful with edge cases such as when the delimiter does not exist in the string (`indexOf` returns `-1`). More importantly, for multi-token splits like `split("\\.")` on a version string, manual slicing can incorrectly include subsequent delimiters if not carefully bounded. For example, `trimmed.substring(dotIndex2 + 1)` on `"1.19.4.1"` includes the rest of the string `"4.1"`, failing a digit-only check.
**Action:** Always handle missing delimiters and edge case bounds (like single element inputs and strings with more elements than expected) when manually rolling `String.split` replacements. Ensure variables strictly map to single tokens without embedded delimiters.
